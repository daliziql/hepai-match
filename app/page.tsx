...
function SettingsPage({weights,setWeights,saved,onSave}:{weights:WeightItem[];setWeights:(next:WeightItem[])=>void;saved:boolean;onSave:()=>void}){const active=weights.filter(w=>w.enabled),total=active.reduce((s,w)=>s+w.weight,0)||1;return <section className="settings-page"><div className="page-intro"><span className="mini-label">模型设置 · V0.6</span><h1>把年龄和婚恋阶段放回核心位置</h1><p>男女使用不同年龄曲线；婚史与子女状态分别计算，并设置阶段上限，避免外表或收入完全覆盖现实选择面的收缩。</p></div><div className="age-curve-note"><div><b>女性年龄曲线</b><span>25岁附近优势较集中，30岁后逐步变陡，35岁后选择面明显收窄。</span></div><div><b>男性年龄曲线</b><span>30–35岁相对稳定，40岁后逐步下降，45岁后下降加快。</span></div><div><b>婚史 × 子女</b><span>离异与带娃不重复混为一项；共同生活的子女会产生更强阶段约束。</span></div></div><div className="group-rule-cards four">{modelGroups.map(group=>{const weight=active.filter(w=>group.keys.includes(w.key)).reduce((s,w)=>s+w.weight,0)/total*100;return <div key={group.key} style={{borderTopColor:group.color}}><span>{group.label}</span><strong>{weight.toFixed(0)}%</strong><small>模型影响度 · {group.keys.map(key=>weights.find(w=>w.key===key)?.label).filter(Boolean).join(" · ")}</small></div>})}</div><div className="settings-card"><div className="settings-head"><div><h2>子项影响度</h2><p>影响度决定曲线敏感度；婚恋阶段还会形成综合分上限</p></div><button onClick={onSave}>{saved?"已保存 ✓":"保存设置"}</button></div>{weights.map((w,i)=><div className={`weight-line ${!w.enabled?"off":""}`} key={w.key}><span>{w.label}</span><input aria-label={`${w.label}影响度`} type="range" min="1" max="30" value={w.weight} onChange={e=>setWeights(weights.map((x,n)=>n===i?{...x,weight:Number(e.target.value)}:x))}/><strong>{w.weight}</strong><button aria-label={`${w.enabled?"停用":"启用"}${w.label}`} className={w.enabled?"toggle on":"toggle"} onClick={()=>setWeights(weights.map((x,n)=>n===i?{...x,enabled:!x.enabled}:x))}><i/></button></div>)}</div><div className="research-card"><div><span>模型逻辑</span><h2>选择面不等于个人价值，也不等于再婚意愿</h2></div><ul><li><b>年龄是强筛选项</b><p>婚恋平台行为中，男性更集中寻找较年轻女性；女性通常接受略年长男性，因此两条曲线不能对称。</p></li><li><b>婚史形成市场分层</b><p>全国样本研究显示，初婚身份相对再婚身份具有婚姻流动优势，婚史会改变可进入的候选池。</p></li><li><b>子女状态单独判断</b><p>是否有子女、是否共同生活会带来不同约束。再婚率还受本人意愿和降低标准影响，不能直接当成受欢迎程度。</p></li></ul><div className="source-links"><a href="https://arxiv.org/abs/1401.5710" target="_blank" rel="noreferrer">中国婚恋网站行为研究</a><a href="https://journals.sagepub.com/doi/10.1177/0192513X231155603" target="_blank" rel="noreferrer">上海线上择偶年龄研究</a><a href="https://www.demographic-research.org/articles/volume/41/3" target="_blank" rel="noreferrer">初婚与再婚匹配研究</a><a href="https://link.springer.com/article/10.1186/s40711-015-0023-9" target="_blank" rel="noreferrer">CFPS再婚影响因素</a></div><p className="assumption-note">城市仅分为一线与其他两档，不独立计分，只对收入与储蓄基线做轻微校准。年龄和婚恋阶段曲线仍属于工程估计，后续需要真实匿名匹配数据继续校准。</p></div></section>}
