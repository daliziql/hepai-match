...
function ResultHero({profile,weights}:{profile:Profile;weights:WeightItem[]}){const result=useMemo(()=>calculate(profile,weights),[profile,weights]);const[showParts,setShowParts]=useState(false);const top=[...result.dimensions].sort((a,b)=>b.score-a.score)[0],total=result.dimensions.reduce((s,x)=>s+x.weight,0)||1;return <section className="result-hero"><div className="result-copy"><span className="mini-label">你的条件画像</span><h1>{result.percentile>=82?"优势清晰型":result.percentile>=65?"稳健平衡型":"成长潜力型"}</h1><p>你的综合条件呈现出稳定、均衡的基础。<b>{top?.label}</b>是当前最鲜明的优势。</p><div className="result-actions"><button onClick={()=>document.querySelector("#profile-editor")?.scrollIntoView({behavior:"smooth"})}>调整我的资料</button><span>同口径：{profile.gender} · {profile.city} · 25–34 岁</span></div></div><div className="result-visual"><div className="orbit one"/><div className="orbit two"/><div className="avatar-shape"><i/><b/><span/></div></div><div className={`score-summary ${showParts?"parts-view":""}`}>{showParts?<><div className="score-mode-head"><small>分项得分</small><button onClick={()=>setShowParts(false)}>← 返回总分</button></div><div className="score-parts">{result.dimensions.map(item=><div key={item.key}><span>{item.label}<small>权重 {(item.weight/total*100).toFixed(1)}%</small></span><i><b style={{width:`${item.score}%`}}/></i><strong>{item.score}</strong></div>)}</div><p className="formula-note">各单项分按权重加总后得到综合分。</p></>:<><div className="score-mode-head"><small>综合条件分</small><button onClick={()=>setShowParts(true)}>查看分项 →</button></div><strong>{result.score}</strong><span>/100</span><div className="percent-bar"><i style={{width:`${result.percentile}%`}}/></div><p>高于同口径人群的 <b>{result.percentile}%</b></p></>}</div></section>}
