...
const weightsSeed: WeightItem[] = [
  ...
  {key:"income",label:"当前收入",weight:15,enabled:true},{key:"savings",label:"存款与资产",weight:10,enabled:true},{key:"family",label:"家庭支持",weight:7,enabled:true},
...
const modelGroups = [
  {key:"looks",label:"外观条件",keys:["appearance","height"] as ScoreKey[],color:"#4298a0"},
  {key:"finance",label:"经济条件",keys:["income","savings","family"] as ScoreKey[],color:"#826aa8"},
  {key:"stage",label:"婚恋阶段",keys:["age","marital","children"] as ScoreKey[],color:"#d66f69"},
...
function calculate(p:Profile,weights:WeightItem[]):ScoreResult{const active=weights.filter(w=>w.enabled),total=active.reduce((s,w)=>s+w.weight,0)||1;const dimensions:DimensionResult[]=active.map(w=>{const score=Math.round(metricScore(w.key,p));return{...w,score,level:rarityLevel(score)}});const groups=modelGroups.map(group=>{const items=dimensions.filter(item=>group.keys.includes(item.key)),weight=items.reduce((s,item)=>s+item.weight,0),groupScore=Math.round(normalAggregate(items));return{...group,weight:weight/total*100,score:groupScore}});const groupInputs=groups.filter(g=>g.weight>0).map(g=>({score:g.score,weight:g.weight}));const base=normalAggregate(groupInputs);const appearanceScore=dimensions.find(x=>x.key==="appearance")?.score??50,incomeScore=dimensions.find(x=>x.key==="income")?.score??50,criticalZ=[percentileZ(appearanceScore),percentileZ(incomeScore)],bestCritical=Math.max(...criticalZ),worstCritical=Math.min(...criticalZ);const positiveExtreme=Math.max(0,bestCritical-.6745)*.5,negativeExtreme=Math.min(0,worstCritical+.6745)*.55;const score=Math.round(zPercent(percentileZ(base)+positiveExtreme+negativeExtreme));return{score,percentile:score,dimensions,groups}}
...
function itemInfo(key:ScoreKey|null,p:Profile,weights:WeightItem[]){if(!key)return"该项用于确定比较人群，让结果更贴近你的现实环境，本身不直接计分。";const w=weights.find(x=>x.key===key);if(!w?.enabled)return"该项当前已停用，不参与综合稀缺度计算。";const score=Math.round(metricScore(key,p)),level=rarityLevel(score);const basis=key==="age"?` 当前使用${p.gender}性年龄曲线，年龄跨过关键阶段后选择面会明显变化。`:key==="marital"?" 婚史会改变可进入的候选范围，并与年龄、子女状态共同作用。":key==="children"?" 子女是否共同生活会带来不同的时间、经济和家庭整合成本。":key==="height"?" 会结合同性别平均身高判断相对位置。":key==="income"||key==="savings"?` 会结合${p.city}的现实水平判断相对位置。`:"";return`当前单项位置 ${score}/100 · ${level}。${basis}这是婚恋选择面的估计，不代表个人价值。`}
...
function SettingsPage({weights,setWeights,saved,onSave}:{weights:WeightItem[];setWeights:(next:WeightItem[])=>void;saved:boolean;onSave:()=>void}){
  ...
  return <section className="settings-page"><div className="page-intro"><span className="mini-label">模型设置 · V0.9</span><h1>用标准正态分布统一所有条件</h1><p>每个单项先定位到人群百分位，再转换为标准正态分布的 Z 值；完成分组与综合计算后，重新映射回0–100的人群位置。</p></div><div className="age-curve-note"><div><b>统一统计尺度</b><span>50分对应正态分布中心，约84分对应高于平均1个标准差，约98分对应高于平均2个标准差。</span></div><div><b>两端对称处理</b><span>非常优秀和非常弱都会离开分布中心，极端优势与极端短板按照同一套正态尺度放大。</span></div><div><b>条件先转百分位</b><span>收入本身并非正态分布，因此先按城市收入曲线得到百分位，再进行正态标准化，避免直接套用错误分布。</span></div></div><div className="group-rule-cards four">{modelGroups.map(group=>{const weight=active.filter(w=>group.keys.includes(w.key)).reduce((s,w)=>s+w.weight,0)/total*100;return <div key={group.key} style={{borderTopColor:group.color}}><span>{group.label}</span><strong>{weight.toFixed(0)}%</strong><small>模型影响度 · {group.keys.map(key=>weights.find(w=>w.key===key)?.label).filter(Boolean).join(" · ")}</small></div>})}</div><div className="settings-card"><div className="settings-head"><div><h2>子项影响度</h2><p>影响度作用于标准化后的 Z 值，极端外表与收入仍会被额外识别</p></div><button onClick={onSave}>{saved?"已保存 ✓":"保存设置"}</button></div>{weights.map((w,i)=><div className={`weight-line ${!w.enabled?"off":""}`} key={w.key}><span>{w.label}</span><input aria-label={`${w.label}影响度`} type="range" min="1" max="30" value={w.weight} onChange={e=>setWeights(weights.map((x,n)=>n===i?{...x,weight:Number(e.target.value)}:x))}/><strong>{w.weight}</strong><button aria-label={`${w.enabled?"停用":"启用"}${w.label}`} className={w.enabled?"toggle on":"toggle"} onClick={()=>setWeights(weights.map((x,n)=>n===i?{...x,enabled:!x.enabled}:x))}><i/></button></div>)}</div><div className="research-card"><div><span>模型逻辑</span><h2>先标准化，再综合，最后还原人群位置</h2></div><ul><li><b>百分位转 Z 值</b><p>不同单位的条件先变成人群百分位，再通过标准正态分布反函数转换，之后才具备可比较的尺度。</p></li><li><b>双向极值保留</b><p>高位和低位在正态分布中分别表现为正、负 Z 值，非常好或非常差都不会被普通条件轻易稀释。</p></li><li><b>结果回到百分位</b><p>综合 Z 值通过正态分布函数还原为0–100分，因此最终分仍可以直观理解为相对人群位置。</p></li></ul><div className="source-links"><a href="https://arxiv.org/abs/1401.5710" target="_blank" rel="noreferrer">中国婚恋网站行为研究</a><a href="https://journals.sagepub.com/doi/10.1177/0192513X231155603" target="_blank" rel="noreferrer">上海线上择偶年龄研究</a><a href="https://www.demographic-research.org/articles/volume/41/3" target="_blank" rel="noreferrer">初婚与再婚匹配研究</a><a href="https://link.springer.com/article/10.1186/s40711-015-0023-9" target="_blank" rel="noreferrer">CFPS再婚影响因素</a></div><p className="assumption-note">正态分布用于统一百分位与综合尺度，并不假设收入等原始数据天然服从正态分布。城市只轻微校准收入与储蓄基线，具体曲线仍需真实匿名数据持续校准。</p></div></section>
}
