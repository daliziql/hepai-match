...
function scarcityStrength(score:number){return-Math.log10(Math.max(.001,1-clamp(score,0,99.9)/100))}
function scarcityPercent(strength:number){return clamp(100*(1-Math.pow(10,-strength)),1,99.8)}
function nonlinearAggregate(items:Array<{score:number;weight:number}>){if(!items.length)return 0;const total=items.reduce((s,x)=>s+x.weight,0)||1;const strengths=items.map(x=>({...x,strength:scarcityStrength(x.score)}));const average=strengths.reduce((s,x)=>s+x.strength*x.weight,0)/total;const strongest=Math.max(...strengths.map(x=>x.strength)),weakest=Math.min(...strengths.map(x=>x.strength));const standout=Math.max(0,strongest-average)*.18;const longTail=Math.max(0,strongest-1)*.12;const balance=Math.max(0,weakest-.4)*.03;return scarcityPercent(average+standout+longTail+balance)}
function calculate(p:Profile,weights:WeightItem[]):ScoreResult{const active=weights.filter(w=>w.enabled),total=active.reduce((s,w)=>s+w.weight,0)||1;const dimensions:DimensionResult[]=active.map(w=>{const score=Math.round(metricScore(w.key,p));return{...w,score,level:rarityLevel(score)}});const groups=modelGroups.map(group=>{const items=dimensions.filter(item=>group.keys.includes(item.key)),weight=items.reduce((s,item)=>s+item.weight,0);let groupScore=Math.round(nonlinearAggregate(items));if(group.key==="stage"&&items.length){const average=items.reduce((s,item)=>s+item.score*item.weight,0)/(weight||1),lowest=Math.min(...items.map(x=>x.score));groupScore=Math.round(average*.55+lowest*.45)}return{...group,weight:weight/total*100,score:groupScore}});const groupInputs=groups.filter(g=>g.weight>0).map(g=>({score:g.score,weight:g.weight}));const strongest=dimensions.reduce((best,item)=>item.score>best.score?item:best,dimensions[0]??{score:50} as DimensionResult);const base=nonlinearAggregate(groupInputs);const appearanceScore=dimensions.find(x=>x.key==="appearance")?.score??50,incomeScore=dimensions.find(x=>x.key==="income")?.score??50;const criticalTail=Math.max(0,scarcityStrength(appearanceScore)-scarcityStrength(72))*2.8+Math.max(0,scarcityStrength(incomeScore)-scarcityStrength(72))*1.6;const rareAdvantage=Math.max(0,scarcityStrength(strongest.score)-scarcityStrength(92))*.8+criticalTail;const stage=groups.find(g=>g.key==="stage");const parentingInteraction=p.marital!=="未婚"&&p.children==="有子女，共同生活"?(p.gender==="女"&&p.age>=38?6:p.gender==="男"&&p.age>=45?4:2):0;const stageThreshold=stage?.weight?45+stage.score*.55-parentingInteraction:99;const candidate=base+rareAdvantage;const retention=stage?.weight?clamp(.25+stage.score/200,.3,.72):1;const softened=candidate<=stageThreshold?candidate:stageThreshold+(candidate-stageThreshold)*retention;const score=Math.round(clamp(softened,1,99));return{score,percentile:score,dimensions,groups}}
...
function SettingsPage({weights,setWeights,saved,onSave}:{weights:WeightItem[];setWeights:(next:WeightItem[])=>void;saved:boolean;onSave:()=>void}){const active=weights.filter(w=>w.enabled),total=active.reduce((s,w)=>s+w.weight,0)||1;return <section className="settings-page"><div className="page-intro"><span className="mini-label">模型设置 · V0.7</span><h1>让高位优势真正体现稀缺程度</h1><p>单项百分位会先转换成长尾稀缺强度再参与汇总；婚恋阶段只压缩高分收益，不再把外表或收入直接锁死在某个总分。</p></div><div className="age-curve-note"><div><b>长尾稀缺</b><span>外观7分约为前10%，8分约为前2%；计算会体现候选人数约5倍的差异，而不只是8个百分位。</span></div><div><b>阶段软约束</b><span>年龄、婚史和子女仍会明显压缩选择面，但任何真实优势继续提升时，综合分都会保留变化。</span></div><div><b>男女年龄曲线</b><span>女性30岁后逐步变陡；男性30–35岁相对稳定，40岁后逐步下降。</span></div></div><div className="group-rule-cards four">{modelGroups.map(group=>{const weight=active.filter(w=>group.keys.includes(w.key)).reduce((s,w)=>s+w.weight,0)/total*100;return <div key={group.key} style={{borderTopColor:group.color}}><span>{group.label}</span><strong>{weight.toFixed(0)}%</strong><small>模型影响度 · {group.keys.map(key=>weights.find(w=>w.key===key)?.label).filter(Boolean).join(" · ")}</small></div>})}</div><div className="settings-card"><div className="settings-head"><div><h2>子项影响度</h2><p>影响度决定曲线敏感度；婚恋阶段会压缩、但不会截断优势收益</p></div><button onClick={onSave}>{saved?"已保存 ✓":"保存设置"}</button></div>{weights.map((w,i)=><div className={`weight-line ${!w.enabled?"off":""}`} key={w.key}><span>{w.label}</span><input aria-label={`${w.label}影响度`} type="range" min="1" max="30" value={w.weight} onChange={e=>setWeights(weights.map((x,n)=>n===i?{...x,weight:Number(e.target.value)}:x))}/><strong>{w.weight}</strong><button aria-label={`${w.enabled?"停用":"启用"}${w.label}`} className={w.enabled?"toggle on":"toggle"} onClick={()=>setWeights(weights.map((x,n)=>n===i?{...x,enabled:!x.enabled}:x))}><i/></button></div>)}</div><div className="research-card"><div><span>模型逻辑</span><h2>选择面不等于个人价值，也不等于再婚意愿</h2></div><ul><li><b>优势按人数差计算</b><p>越接近顶端，每提升一个等级对应的人数差越大，外表与收入的高位提升会更明显。</p></li><li><b>阶段仍然重要</b><p>年龄、婚史和子女会降低同等优势能够转化出的选择面，但不再造成总分完全不动。</p></li><li><b>匹配难度独立判断</b><p>目标外表与收入越稀缺，候选越少；难度仍会结合双方差距与目标的稀缺门槛。</p></li></ul><div className="source-links"><a href="https://arxiv.org/abs/1401.5710" target="_blank" rel="noreferrer">中国婚恋网站行为研究</a><a href="https://journals.sagepub.com/doi/10.1177/0192513X231155603" target="_blank" rel="noreferrer">上海线上择偶年龄研究</a><a href="https://www.demographic-research.org/articles/volume/41/3" target="_blank" rel="noreferrer">初婚与再婚匹配研究</a><a href="https://link.springer.com/article/10.1186/s40711-015-0023-9" target="_blank" rel="noreferrer">CFPS再婚影响因素</a></div><p className="assumption-note">城市仅分为一线与其他两档，不独立计分，只轻微校准收入与储蓄基线。模型表达的是相对选择面，仍需用真实匿名匹配数据持续校准。</p></div></section>}

export default function Home(){const[page,setPage]=useState<"profile"|"match"|"settings">("profile");const[profile,setProfile]=useState(profileSeed);const[targets,setTargets]=useState(targetSeeds);const[active,setActive]=useState<TargetKind>("balanced");const[weights,setWeights]=useState(weightsSeed);const[saved,setSaved]=useState(false);useEffect(()=>{const raw=localStorage.getItem("hepan-config-v5");if(raw)try{const old=JSON.parse(raw) as WeightItem[];setWeights(weightsSeed.map(seed=>{const stored=old.find(item=>item.key===seed.key);return stored?{...seed,weight:stored.weight,enabled:stored.enabled}:seed}))}catch{}},[]);const own=calculate(profile,weights);const updateTarget=(p:Profile)=>setTargets({...targets,[active]:p});const save=()=>{localStorage.setItem("hepan-config-v5",JSON.stringify(weights));setSaved(true);setTimeout(()=>setSaved(false),1600)};
