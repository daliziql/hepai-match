...
function marketSignal(score:number){const p=clamp(score,.1,99.9);return p>=50?Math.log10(50/(100-p)):-Math.log10(50/p)}
function signalPercent(signal:number){return clamp(signal>=0?100-50/Math.pow(10,signal):50/Math.pow(10,-signal),1,99)}
function nonlinearAggregate(items:Array<{score:number;weight:number}>){if(!items.length)return 0;const total=items.reduce((s,x)=>s+x.weight,0)||1;const signals=items.map(x=>({...x,signal:marketSignal(x.score)}));const average=signals.reduce((s,x)=>s+x.signal*x.weight,0)/total;const strongest=Math.max(...signals.map(x=>x.signal)),weakest=Math.min(...signals.map(x=>x.signal));const positive=Math.max(0,strongest-average)*.2+Math.max(0,strongest-.7)*.12;const negative=Math.min(0,weakest-average)*.24+Math.min(0,weakest+.7)*.18;return signalPercent(average+positive+negative)}
function calculate(p:Profile,weights:WeightItem[]):ScoreResult{const active=weights.filter(w=>w.enabled),total=active.reduce((s,w)=>s+w.weight,0)||1;const dimensions:DimensionResult[]=active.map(w=>{const score=Math.round(metricScore(w.key,p));return{...w,score,level:rarityLevel(score)}});const groups=modelGroups.map(group=>{const items=dimensions.filter(item=>group.keys.includes(item.key)),weight=items.reduce((s,item)=>s+item.weight,0),groupScore=Math.round(nonlinearAggregate(items));return{...group,weight:weight/total*100,score:groupScore}});const groupInputs=groups.filter(g=>g.weight>0).map(g=>({score:g.score,weight:g.weight}));const base=nonlinearAggregate(groupInputs);const appearanceScore=dimensions.find(x=>x.key==="appearance")?.score??50,incomeScore=dimensions.find(x=>x.key==="income")?.score??50;const criticalSignals=[marketSignal(appearanceScore),marketSignal(incomeScore)],bestCritical=Math.max(...criticalSignals),worstCritical=Math.min(...criticalSignals);const positiveExtreme=Math.max(0,bestCritical-.7)*.38,negativeExtreme=Math.min(0,worstCritical+.7)*.48;const score=Math.round(signalPercent(marketSignal(base)+positiveExtreme+negativeExtreme));return{score,percentile:score,dimensions,groups}}
...
function SettingsPage({weights,setWeights,saved,onSave}:{weights:WeightItem[];setWeights:(next:WeightItem[])=>void;saved:boolean;onSave:()=>void}){const active=weights.filter(w=>w.enabled),total=active.reduce((s,w)=>s+w.weight,0)||1;return <section className="settings-page"><div className="page-intro"><span className="mini-label">模型设置 · V0.8</span><h1>同时识别极端优势与极端短板</h1><p>所有单项都以50分为中点做双向长尾换算：非常优秀会强力拉高结果，非常弱也会显著拉低结果，不再只奖励优秀端。</p></div><div className="age-curve-note"><div><b>优势可以破局</b><span>非常好看或非常富裕本身就是强市场信号，即使婚恋阶段不理想，也能形成较高综合位置。</span></div><div><b>短板不可稀释</b><span>非常丑或非常贫穷会形成强负向信号，一组普通条件不能轻易把它平均掉。</span></div><div><b>阶段回归影响项</b><span>年龄、婚史和子女继续参与综合判断，但不再对极端优势设置额外上限。</span></div></div><div className="group-rule-cards four">{modelGroups.map(group=>{const weight=active.filter(w=>group.keys.includes(w.key)).reduce((s,w)=>s+w.weight,0)/total*100;return <div key={group.key} style={{borderTopColor:group.color}}><span>{group.label}</span><strong>{weight.toFixed(0)}%</strong><small>模型影响度 · {group.keys.map(key=>weights.find(w=>w.key===key)?.label).filter(Boolean).join(" · ")}</small></div>})}</div><div className="settings-card"><div className="settings-head"><div><h2>子项影响度</h2><p>影响度决定常规变化，极端区间还会触发双向长尾放大</p></div><button onClick={onSave}>{saved?"已保存 ✓":"保存设置"}</button></div>{weights.map((w,i)=><div className={`weight-line ${!w.enabled?"off":""}`} key={w.key}><span>{w.label}</span><input aria-label={`${w.label}影响度`} type="range" min="1" max="30" value={w.weight} onChange={e=>setWeights(weights.map((x,n)=>n===i?{...x,weight:Number(e.target.value)}:x))}/><strong>{w.weight}</strong><button aria-label={`${w.enabled?"停用":"启用"}${w.label}`} className={w.enabled?"toggle on":"toggle"} onClick={()=>setWeights(weights.map((x,n)=>n===i?{...x,enabled:!x.enabled}:x))}><i/></button></div>)}</div><div className="research-card"><div><span>模型逻辑</span><h2>强项能破局，硬伤也会形成门槛</h2></div><ul><li><b>双向长尾转换</b><p>高位与低位都按现实人数差放大。7到8分的优势、3到2分的短板，都比普通区间变化更大。</p></li><li><b>外表与收入优先</b><p>外表和当前收入的极端值会得到额外识别，反映现实初筛中的强吸引力或强劝退效应。</p></li><li><b>阶段不再封顶</b><p>年龄、婚史和子女仍占重要位置，但只贡献自身信号，不再阻止其他优势继续提高总分。</p></li></ul><div className="source-links"><a href="https://arxiv.org/abs/1401.5710" target="_blank" rel="noreferrer">中国婚恋网站行为研究</a><a href="https://journals.sagepub.com/doi/10.1177/0192513X231155603" target="_blank" rel="noreferrer">上海线上择偶年龄研究</a><a href="https://www.demographic-research.org/articles/volume/41/3" target="_blank" rel="noreferrer">初婚与再婚匹配研究</a><a href="https://link.springer.com/article/10.1186/s40711-015-0023-9" target="_blank" rel="noreferrer">CFPS再婚影响因素</a></div><p className="assumption-note">城市只做轻微收入与储蓄校准。极端区间代表现实选择面的强信号，而不是对个人价值的评价；具体强度仍需真实匿名匹配数据持续校准。</p></div></section>}
